 ...
 *
 * Note: the backend derives `totalSpent` from approved events but still returns
 * a hardcoded `annualAllocation` (₦2.5M), so the allocation and the balance
 * computed from it are not configurable per branch yet.
 */
...
export type ExitClearance = {
  ...
  pastoralRelease?: PastoralRelease
