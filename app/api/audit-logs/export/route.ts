...
function getBackendAuditLogsExportUrl(): string | null {
  const baseUrl = process.env.BACKEND_API_URL?.replace(/\/+$/, "");
  if (baseUrl) {
    return `${baseUrl}/api/v1/audit-logs/export`;
  }
  ...
  if (loginUrl.includes("/auth/login")) {
    return loginUrl.replace("/auth/login", "/audit-logs/export");
  }

  if (loginUrl.endsWith("/login")) {
    return loginUrl.replace(/\/login$/, "/audit-logs/export");
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    const backendUrl = getBackendAuditLogsExportUrl();
    if (!backendUrl) {
      return applyCors(
    ...
    return applyCors(
      NextResponse.json({ success: false, message: "Server error occurred while exporting audit logs." }, { status: 502 }),
      req
    );
  }
}
