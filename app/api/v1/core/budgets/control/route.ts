...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = `${baseUrl.replace(/\/+$/, "")}/api/v1/budgets/control${queryString ? `?${queryString}` : ""}`
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: responseData?.message ?? "Failed to fetch budget control dashboard",
          },
    ...
    console.error("Dashboard budget control proxy error:", error)
    return applyCors(
      NextResponse.json(
