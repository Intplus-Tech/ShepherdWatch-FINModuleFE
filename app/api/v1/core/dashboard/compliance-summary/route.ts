...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = `${baseUrl.replace(/\/+$/, "")}/api/v1/compliance/summary${queryString ? `?${queryString}` : ""}`
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: responseData?.message ?? "Failed to fetch compliance summary",
          },
    ...
    console.error("Dashboard compliance-summary proxy error:", error)
    return applyCors(
      NextResponse.json(
