...
function getBackendUrl(searchParams: URLSearchParams): string {
  ...
  return `${baseUrl}/api/v1/dashboard/budget-vs-actuals${queryString ? `?${queryString}` : ""}`;
}
  ...
  try {
    ...
    if (isJson) {
      try {
        ...
        return applyCors(
          NextResponse.json(
            { success: false, message: "Invalid response received from budget vs actuals service" },
            { status: 502 }
          ),
          req
        );
    ...
    console.error("Dashboard budget-vs-actuals proxy error:", error);
    return applyCors(
