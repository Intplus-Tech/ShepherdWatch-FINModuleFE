...
function buildBackendUrl(requisitionId: string): string {
  ...
  return `${baseUrl.replace(/\/+$/, "")}/api/v1/requisitions/${requisitionId}/approve`
}
...
export async function PATCH(req: NextRequest, context: { params: { requisitionId: string } }) {
  try {
    ...
    if (!["approved", "declined"].includes(action)) {
      return applyCors(
        NextResponse.json({ success: false, message: "Action must be approved or declined." }, { status: 400 }),
        req
      )
    }
