...
function buildBackendUrl(requisitionId: string): string {
  ...
  return `${baseUrl.replace(/\/+$/, "")}/api/v1/requisitions/${requisitionId}/budget-context`
}

export async function GET(req: NextRequest, context: { params: { requisitionId: string } }) {
  try {
    ...
    console.error("Get requisition budget context proxy error:", error)
    return applyCors(
