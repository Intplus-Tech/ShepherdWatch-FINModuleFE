...
function buildBackendUrl(requisitionId: string): string {
  ...
  return `${baseUrl.replace(/\/+$/, "")}/api/v1/requisitions/${requisitionId}/override`
}
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ requisitionId: string }> }) {
  try {
    ...
    const overrideJustification = String(body?.overrideJustification ?? "").trim()
    if (!overrideJustification) {
      return applyCors(
        NextResponse.json(
          { success: false, message: "overrideJustification is required." },
          { status: 400 }
        ),
        req
      )
    ...
    console.error("Authorize requisition override proxy error:", error)
    return applyCors(
