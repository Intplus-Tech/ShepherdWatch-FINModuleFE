...
function buildBackendUrl(search: string): string {
  ...
  const url = new URL(`${baseUrl.replace(/\/+$/, "")}/api/v1/core/financial/maintenance-tasks`)
  if (search) {
    url.search = search
  }
  return url.toString()
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to fetch maintenance tasks" },
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to create maintenance task" },
          { status: backendResponse.status || 502 }
        ),
        req
      )
