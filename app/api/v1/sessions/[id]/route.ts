...
function getBackendSessionUrl(id: string): string | null {
  const baseUrl = process.env.BACKEND_API_URL?.replace(/\/+$/, "");
  if (baseUrl) {
    return `${baseUrl}/api/v1/sessions/${encodeURIComponent(id)}`;
  }
  ...
  if (loginUrl.includes("/auth/login")) {
    return loginUrl.replace("/auth/login", `/sessions/${encodeURIComponent(id)}`);
  }

  if (loginUrl.endsWith("/login")) {
    return loginUrl.replace(/\/login$/, `/sessions/${encodeURIComponent(id)}`);
  }
...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const backendUrl = getBackendSessionUrl(id);
    if (!backendUrl) {
      return applyCors(
    ...
    return applyCors(
      NextResponse.json(
        { success: false, message: "Server error occurred while fetching session details." },
        { status: 502 }
      ),
      req
    );
