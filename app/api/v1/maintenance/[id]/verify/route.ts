...
export async function POST(
  ...
  try {
    ...
    const url = new URL(
      `${baseUrl.replace(/\/+$/, "")}/api/v1/maintenance/${recordId}/verify`
    )
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: payload?.message ?? "Unable to verify maintenance record",
          },
    ...
    console.error("Verify maintenance proxy error:", error)
    return applyCors(
      NextResponse.json(
