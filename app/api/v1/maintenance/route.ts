...
export async function POST(req: NextRequest) {
  try {
    ...
    const url = new URL(`${baseUrl.replace(/\/+$/, "")}/api/v1/maintenance`)
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: payload?.message ?? "Unable to create maintenance record",
          },
    ...
    console.error("Create maintenance proxy error:", error)
    return applyCors(
      NextResponse.json(
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: payload?.message ?? "Unable to fetch maintenance records",
          },
          { status: backendResponse.status || 502 }
        ),
        req
    ...
    console.error("List maintenance proxy error:", error)
    return applyCors(
      NextResponse.json(
        { success: false, message: "Internal server error" },
        { status: 500 }
