...
export async function GET(req: NextRequest) {
  try {
    ...
    const backendUrl = `${baseUrl}/dashboard/expense-distribution${
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Failed to fetch expense distribution", ...(payload && typeof payload === "object" ? payload : {}) },
    ...
    console.error("Dashboard expense distribution proxy error:", error)
    return applyCors(
