...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = `${baseUrl}/dashboard/transactions-summary${queryString ? `?${queryString}` : ""}`
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: responseData?.message ?? "Failed to fetch transactions summary",
          },
    ...
    console.error("Dashboard transactions-summary proxy error:", error)
    return applyCors(
      NextResponse.json(
