...
function buildBackendUrl(): string {
  ...
  return `${baseUrl}/financial/budget-proposals`
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to create budget proposal", ...(payload && typeof payload === "object" ? payload : {}) },
    ...
    console.error("Create budget proposal proxy error:", error)
    return applyCors(
