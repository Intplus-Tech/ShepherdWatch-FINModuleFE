...
export async function GET(req: NextRequest) {
  try {
    ...
    const backendUrl = new URL(`${baseUrl}/requisitions/inbox`)
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to fetch requisition inbox", ...(payload && typeof payload === "object" ? payload : {}) },
