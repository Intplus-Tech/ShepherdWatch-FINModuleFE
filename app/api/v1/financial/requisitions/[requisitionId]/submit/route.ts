...
function buildBackendUrl(requisitionId: string): string {
  ...
  return `${baseUrl}/requisitions/${requisitionId}/submit`
}
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ requisitionId: string }> }) {
  try {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to submit requisition", ...(payload && typeof payload === "object" ? payload : {}) },
    ...
    console.error("Submit requisition proxy error:", error)
    return applyCors(
