...
function buildBackendUrl(search: string): string {
  ...
  const url = new URL(`${baseUrl}/financial/reports/statutory`)
  if (search) {
    url.search = search
  }
  return url.toString()
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to fetch statutory report", ...(payload && typeof payload === "object" ? payload : {}) },
