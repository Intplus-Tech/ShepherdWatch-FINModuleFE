...
function buildBackendUrl(search: string): string {
  ...
  const url = new URL(`${baseUrl}/financial/branch-aggregation/branches`)
  if (search) {
    url.search = search
  }
  return url.toString()
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { res: backendResponse } = await executeWithRefreshRetry(req, (backendToken) =>
      fetch(buildBackendUrl(req.nextUrl.search), {
        method: "GET",
        headers: {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to fetch branch summaries", ...(payload && typeof payload === "object" ? payload : {}) },
    ...
    console.error("Get branch summaries proxy error:", error)
    return applyCors(
