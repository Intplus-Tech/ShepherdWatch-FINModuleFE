...
function buildBackendUrl(search: string): string {
  ...
  const url = new URL(`${baseUrl}/compliance/summary`)
  if (search) {
    url.search = search
  }
  return url.toString()
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to fetch compliance summary", ...(payload && typeof payload === "object" ? payload : {}) },
    ...
    console.error("Get compliance summary proxy error:", error)
    return applyCors(
