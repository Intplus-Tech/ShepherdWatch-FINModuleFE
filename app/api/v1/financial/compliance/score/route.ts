...
function buildBackendUrl(search: string): string {
  ...
  const url = new URL(`${baseUrl}/financial/compliance/score`)
  if (search) {
    url.search = search
  }
  return url.toString()
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to fetch compliance score", ...(payload && typeof payload === "object" ? payload : {}) },
    ...
    console.error("Get compliance score proxy error:", error)
    return applyCors(
