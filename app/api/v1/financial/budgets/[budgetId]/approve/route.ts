...
function buildBackendUrl(budgetId: string): string {
  ...
  return `${baseUrl}/budgets/${budgetId}/approve`
}
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ budgetId: string }> }) {
  try {
    ...
    const status = String((body as Record<string, unknown> | null)?.status ?? "").trim().toLowerCase()
    if (!["approved", "rejected", "revision"].includes(status)) {
      return applyCors(
        NextResponse.json(
          { success: false, message: "Status must be one of: approved, rejected, revision." },
          { status: 400 }
        ),
        req
      )
