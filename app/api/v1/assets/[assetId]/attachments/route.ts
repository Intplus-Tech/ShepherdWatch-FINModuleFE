...
function buildBackendUrl(assetId: string): string {
  const baseUrl = getBackendApiUrl()
  return `${baseUrl}/assets/${assetId}/attachments`
}
...
export async function PATCH(
  ...
  try {
    ...
    const backendResponse = await fetch(buildBackendUrl(assetId), {
      method: "PATCH",
      headers: {
    ...
    console.error("Update asset attachments proxy error:", error)
    return applyCors(
