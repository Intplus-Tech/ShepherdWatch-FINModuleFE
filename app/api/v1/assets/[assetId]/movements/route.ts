...
export async function GET(
  ...
  try {
    ...
    const url = new URL(`${baseUrl}/assets/${assetId}/movements`)
    ...
    console.error("List asset movement history proxy error:", error)
    return applyCors(
      NextResponse.json(
...
export async function POST(
  ...
  try {
    ...
    console.error("Record asset movement proxy error:", error)
    return applyCors(
      NextResponse.json(
        { success: false, message: "Internal server error" },
        { status: 500 }
