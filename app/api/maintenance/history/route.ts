...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = new URL(`${baseUrl.replace(/\/+$/, "")}/api/v1/maintenance/history`)
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: payload?.message ?? "Unable to fetch maintenance history",
          },
    ...
    console.error("Maintenance history proxy error:", error)
    return applyCors(
      NextResponse.json(
