...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = new URL(`${baseUrl.replace(/\/+$/, "")}/api/v1/maintenance/schedule`)
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: payload?.message ?? "Unable to fetch maintenance schedule",
          },
    ...
    console.error("Maintenance schedule proxy error:", error)
    return applyCors(
      NextResponse.json(
