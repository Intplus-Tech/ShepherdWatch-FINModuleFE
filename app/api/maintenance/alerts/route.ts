...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = new URL(`${baseUrl.replace(/\/+$/, "")}/api/v1/maintenance/alerts`)
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: payload?.message ?? "Unable to fetch maintenance alerts",
          },
    ...
    console.error("Maintenance alerts proxy error:", error)
    return applyCors(
      NextResponse.json(
