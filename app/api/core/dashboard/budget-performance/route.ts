...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = `${baseUrl.replace(/\/+$/, "")}/api/v1/dashboard/budget-performance${queryString ? `?${queryString}` : ""}`
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: responseData?.message ?? "Failed to fetch budget performance",
          },
    ...
    console.error("Dashboard budget-performance proxy error:", error)
    return applyCors(
      NextResponse.json(
