...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = `${baseUrl.replace(/\/+$/, "")}/api/v1/dashboard/approval-queue${queryString ? `?${queryString}` : ""}`
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: responseData?.message ?? "Failed to fetch approval queue",
          },
    ...
    console.error("Dashboard approval queue proxy error:", error)
    return applyCors(
      NextResponse.json(
