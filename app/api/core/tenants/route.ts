...
export async function GET(req: NextRequest) {
  try {
    ...
    const backendUrl = buildBackendTenantsUrl(req.nextUrl.search);
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: payload?.message ?? "Unable to fetch tenants",
          },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const backendUrl = buildBackendTenantsUrl("");
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: payload?.message ?? "Unable to create branch",
          },
          { status: backendResponse.status || 502 }
        ),
        req
    ...
    console.error("Create branch proxy error:", error);
    return applyCors(
      NextResponse.json({ success: false, message: "Internal server error" }, { status: 500 }),
      req
    );
