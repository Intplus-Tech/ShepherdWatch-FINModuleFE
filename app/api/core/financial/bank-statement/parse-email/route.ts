...
function buildBackendUrl(): string {
  ...
  return `${baseUrl.replace(/\\/+$/, "")}/api/v1/core/financial/bank-statement/parse-email`
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          { success: false, message: payload?.message ?? "Unable to parse email alert" },
    ...
    console.error("Parse email alert proxy error:", error)
    return applyCors(
