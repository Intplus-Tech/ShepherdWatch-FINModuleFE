...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = new URL(`${baseUrl.replace(/\/+$/, "")}/api/v1/permissions/matrix`)
    ...
    console.error("Get permissions matrix proxy error:", error)
    return applyCors(
      NextResponse.json(
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error("Save permissions matrix proxy error:", error)
    return applyCors(
      NextResponse.json(
        { success: false, message: "Internal server error" },
        { status: 500 }
