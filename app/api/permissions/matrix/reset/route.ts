...
export async function POST(req: NextRequest) {
  try {
    ...
    const url = new URL(`${baseUrl.replace(/\/+$/, "")}/api/v1/permissions/matrix/reset`)
    ...
    if (!backendResponse.ok) {
      return applyCors(
        NextResponse.json(
          {
            success: false,
            message: payload?.message ?? "Unable to reset permissions matrix",
          },
    ...
    console.error("Reset permissions matrix proxy error:", error)
    return applyCors(
      NextResponse.json(
