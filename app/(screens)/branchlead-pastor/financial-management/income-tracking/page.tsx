...
export default function Page() {
  ...
  // The month's figures come from the income-summary endpoint, which returns
  // exactly what these cards show; the change badge from the dashboard summary.
  const { income: monthSummary } = useTransactionSummaries({ branchId: branchId || undefined, month: now.getMonth() + 1, fiscalYear: now.getFullYear() })
  ...
  const monthDelta = pctBadge(trend?.inflowMonth?.delta == null ? null : Number(trend?.inflowMonth?.delta) * 100)
  const unverifiedCount = Number((txTrends as { unverifiedCount?: number } | null)?.unverifiedCount ?? 0)
  const targetPct = monthSummary && monthSummary.budgetTarget > 0 ? Math.min(100, Math.round((monthSummary.totalCollected / monthSummary.budgetTarget) * 100)) : null
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_340px] mb-8">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-[16px] border border-[#E5E7EB] bg-white p-5 shadow-sm">
                ...
                <div className="text-[26.67px] font-black text-[#111827] leading-8 tracking-normal">{monthSummary ? compactNaira(monthSummary.totalCollected) : "—"}</div>
              </div>
              ...
              <div className="rounded-[16px] border border-[#E5E7EB] bg-white p-5 shadow-sm">
                ...
                <div className="text-[26.67px] font-black text-[#111827] leading-8 tracking-normal">{monthSummary ? compactNaira(monthSummary.projected) : "—"}</div>
              </div>
