...
import LeadPastorUsersPage from "@/app/(screens)/branchlead-pastor/users/page"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Switch } from "@/components/ui/switch"
import { ChevronDown, Mail, UserRound, X } from "lucide-react"
import { useAuth } from "@/components/auth/AuthProvider"
import { useInviteUser } from "@/components/hooks/useUsers"
...
export default function Page() {
  ...
  // A branch pastor invites into their own branch — no branch picker needed.
  const branchId = useMemo(() => user?.tenantId ?? user?.tenant?.id ?? "", [user])
  ...
  const handleInvite = async () => {
    ...
    if (!fullName || !email || !role) {
      setError("Please fill out all required fields: name, email, role.")
      return
    }
    if (!branchId) {
      setError("Your account has no branch assigned. Contact an administrator.")
      return
    }
