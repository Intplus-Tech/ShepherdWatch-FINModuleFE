...
import type { RequisitionItem } from "@/components/hooks/useRequisitions"
...
export default function RequisitionDetailModal({
  ...
  const title = requisition.justification?.split(" - ")[0] || "Requisition Request"
  const description =
    requisition.justification?.split(" - ").slice(1).join(" - ") ||
    requisition.justification ||
    "No description provided for this requisition."
  const requesterName = requisition.requestedBy || "Branch Officer"
  const budgetHead = requisition.coaName || "General"
  const documents = requisition.documents ?? []

  return (
    <ModalShell open={open} onClose={onClose} className="max-w-[850px]">
      {/* Close Button Top Right */}
      <button
        onClick={onClose}
        className="absolute top-4 right-4 md:top-5 md:right-5 text-gray-500 hover:text-gray-800 transition-colors z-10"
        aria-label="Close"
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-start md:items-center justify-between gap-3 md:gap-4">
          ...
          <div className="flex bg-white items-center gap-2.5 md:gap-3 border border-[#EEF1F6] rounded-full px-1.5 py-1.5 md:px-1.5 md:py-1.5 shadow-sm shrink-0 pr-3 sm:pr-3 md:pr-4 mx-auto sm:mx-0 w-[90%] sm:w-auto mt-2 sm:mt-0">
            ...
            <div className="flex flex-col justify-center">
              <div className="text-[12px] sm:text-[11px] md:text-[13px] font-[800] text-[#111827] leading-tight">
                {requesterName}
              </div>
              <div className="text-[10px] sm:text-[9.5px] md:text-[11px] font-[600] text-[#6B7280] leading-tight mt-0.5">
                {budgetHead} Dept.
              </div>
            </div>
          </div>
      ...
      <div className="px-5 sm:px-6 md:px-8 pb-5 sm:pb-6 md:pb-8 grid grid-cols-1 sm:grid-cols-5 gap-4 md:gap-6">
        ...
        <div className="sm:col-span-3 flex flex-col gap-4 md:gap-6">
          ...
          <div className="bg-white border border-[#EEF1F6] rounded-[10px] md:rounded-[12px] p-4 sm:p-3.5 md:p-5 shadow-sm">
            <h3 className="text-[13px] sm:text-[12px] md:text-[14px] font-[800] text-[#111827] mb-2 md:mb-3">
              Description
            </h3>
          ...
          <div className="bg-white border border-[#EEF1F6] rounded-[10px] md:rounded-[12px] p-4 sm:p-3.5 md:p-5 shadow-sm flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-3 md:mb-4">
              <h3 className="text-[13px] sm:text-[12px] md:text-[14px] font-[800] text-[#111827]">
                Attachments ({documents.length || 2})
              </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-2 md:gap-3 flex-1">
              {(documents.length
                ? documents.map((doc) => ({
                    name: doc.name,
                    meta: doc.size ?? "Attachment",
                    pdf: doc.name?.toLowerCase().endsWith(".pdf") ?? true,
                  }))
                : [
                    { name: "Sweetwater_Quote_#99...", meta: "1.2 MB • Oct 24", pdf: true },
                    { name: "Old_Cables_Photo.jpg", meta: "2.4 MB • Oct 24", pdf: false },
                  ]
              ).map((file, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-2.5 p-2.5 md:p-3 rounded-[6px] md:rounded-[8px] bg-[#F8FAFC] border border-[#EEF1F6] group transition-colors hover:border-[#D1D5DB]"
                >
                  <div
                    className={`h-8 w-8 md:h-10 md:w-10 shrink-0 rounded-[5px] md:rounded-[6px] flex items-center justify-center ${
                      file.pdf ? "bg-[#FEE2E2]" : "bg-[#E0E7FF]"
                    }`}
                  >
                    {file.pdf ? (
                      <svg
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[11px] sm:text-[10px] md:text-[12px] font-[800] text-[#111827] truncate">
                      {file.name}
                    </div>
                    <div className="text-[10px] sm:text-[9px] md:text-[11px] font-[500] text-[#9CA3AF] mt-0.5 md:mt-0.5">
                      {file.meta}
                    </div>
                  </div>
        ...
        <div className="sm:col-span-2 flex flex-col gap-4 md:gap-6 mt-2 sm:mt-0">
          {/* Amount Card */}
          <div className="bg-white border border-[#EEF1F6] rounded-[10px] md:rounded-[12px] p-4 sm:p-3.5 md:p-5 shadow-sm">
            <h3 className="text-[10px] sm:text-[9.5px] md:text-[11px] font-[800] text-[#9CA3AF] uppercase tracking-wider mb-1.5 md:mb-2">
              Requested Amount
            </h3>
            <div className="text-[24px] sm:text-[22px] md:text-[32px] font-[900] text-[#111827] tracking-tight mb-3 md:mb-4">
              {formatCurrency(requisition.amount || 0)}
            </div>
            <div className="h-[1px] w-full bg-[#EEF1F6] mb-3 md:mb-4"></div>
            <div className="flex items-center justify-between">
              <span className="text-[12px] sm:text-[11px] md:text-[13px] font-[600] text-[#6B7280]">
                Budget Head
              </span>
              <span className="text-[12px] sm:text-[11.5px] md:text-[13px] font-[800] text-[#111827] truncate max-w-[55%] text-right">
                {budgetHead}
              </span>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#EEF1F6] rounded-[10px] md:rounded-[12px] p-4 sm:p-3.5 md:p-5 shadow-sm flex-1 flex flex-col h-[180px] sm:h-auto">
            ...
            <div className="bg-[#F9FAFB] border border-[#EEF1F6] rounded-[6px] md:rounded-[8px] p-2.5 md:p-3 flex-1 flex h-full">
              <textarea
                ...
                placeholder="Add a note to the Accountant"
              ></textarea>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 md:gap-3 w-full sm:w-auto mt-2 sm:mt-0">
          <div className="relative flex-1 sm:flex-none">
            <button
              ...
              className="h-[40px] md:h-[44px] px-3 md:px-4 rounded-[6px] md:rounded-[8px] border border-[#E5E7EB] bg-white text-[#4B5563] text-[12px] md:text-[13px] font-[800] flex items-center justify-center gap-1.5 md:gap-2 hover:bg-[#F9FAFB] transition-colors w-full"
            >
              {priority}
              <ChevronDown className="h-3.5 w-3.5 md:h-4 md:w-4 text-[#9CA3AF]" strokeWidth={2.5} />
            </button>
            {priorityOpen && (
              <div className="absolute bottom-full mb-2 left-0 w-full min-w-[120px] rounded-[8px] border border-[#E5E7EB] bg-white shadow-lg overflow-hidden z-10">
                {PRIORITIES.map((option) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-[13px] font-[700] hover:bg-[#F9FAFB] transition-colors ${
                      option === priority ? "text-[#2563EB]" : "text-[#4B5563]"
                    }`}
                  >
                    {option}
                  </button>
