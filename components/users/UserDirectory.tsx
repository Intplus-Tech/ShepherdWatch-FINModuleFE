...
import { readMatrix, type MatrixEntry } from "@/lib/roles-adapter"
...
export type UserDirectoryProps = {
  /** Role home, e.g. "/director-screen"; used for the Invite link. */
  basePath: string
  /** Scope the list to one branch (a pastor or admin sees their own). */
  branchId?: string
  /** Where "Invite New User" goes; omitted when the role has no invite page. */
  inviteHref?: string
  /** Sibling tabs; omitted when the role has no such page. */
  matrixHref?: string
  auditHref?: string
  /** Moving a user between branches is an organisation-level action. */
  canChangeBranch?: boolean
}

/**
 * User & Role Management: the directory, its row actions and modals, and the
 * role-permissions overview. Shared by every role that manages users; the
 * page around it supplies the sidebar and scope.
 */
export default function UserDirectory({
  ...
  const handleUpdateRole = async () => {
    ...
    setRoleUpdateError(null);
    try {
      const res = await fetch(`${API_V1}/users/${editRoleUserId}/role`, {
        method: "PUT",
        headers: {
  ...
  // The overview reads the live permissions matrix rather than a drawn one.
  const [matrix, setMatrix] = useState<MatrixEntry[]>([])
  useEffect(() => {
    let active = true
    fetch(`${API_V1}/permissions/matrix`, { credentials: "include" })
      .then((r) => (r.ok ? r.json() : null))
      .then((json) => {
        if (active && json) setMatrix(readMatrix(json))
      })
      .catch(() => {
        /* the overview shows nothing granted until the matrix loads */
      })
    return () => {
      active = false
    }
  }, [])
  const OVERVIEW_ROLES: { label: string; keys: string[] }[] = [
    { label: "Director", keys: ["director", "super_admin"] },
    { label: "Pastor", keys: ["branch_pastor", "pastor", "regional_pastor"] },
    { label: "Accountant", keys: ["accountant"] },
    { label: "Admin", keys: ["admin", "branch_admin", "admin_officer"] },
  ]
  const granted = (roleLabel: string, categories: string[], actions: string[]) => {
    const keys = OVERVIEW_ROLES.find((r) => r.label === roleLabel)?.keys ?? []
    return matrix.some(
      (entry) =>
        keys.includes(String(entry.role).toLowerCase()) &&
        entry.permissions.some(
          (p) => categories.some((c) => String(p.category).toLowerCase().includes(c)) && p.actions.some((a) => actions.includes(String(a).toLowerCase()))
        )
    )
  }
  ...
  const handleUpdateBranch = async () => {
    ...
    setBranchUpdateError(null);
    try {
      const res = await fetch(`${API_V1}/users/${changeBranchUserId}/branch`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleResetPassword = async () => {
    ...
    setResetPasswordSuccess(null);
    try {
      const res = await fetch(`${API_V1}/users/${resetPasswordUserId}/reset-password`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        }
  ...
  const handleDeactivate = async () => {
    ...
    setDeactivateSuccess(null);
    try {
      const res = await fetch(`${API_V1}/users/${deactivateUserId}/deactivate`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        }
  ...
  const handleActivate = async () => {
    ...
    setActivateSuccess(null);
    try {
      const res = await fetch(`${API_V1}/users/${activateUserId}/activate`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        }
  ...
  const handleResendInvite = async () => {
    ...
    setResendInviteSuccess(null);
    try {
      const res = await fetch(`${API_V1}/users/${resendInviteUserId}/resend-invite`, {
        method: "POST",
  ...
  const fetchUsers = async (search = "") => {
    setLoading(true)
    try {
      const url = new URL(`${API_V1}/users`, window.location.origin)
          ...
          <section className="rounded-xl border border-[#EEF1F6] bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {inviteHref && (
                  <Button onClick={() => router.push(inviteHref)} className={`${bigText} h-8 rounded-md bg-[#3B5BDB] text-white shadow hover:bg-blue-700`}>
            ...
            <div className={`mt-5 flex items-center gap-6 border-b border-[#EEF1F6] ${bigText}`}>
              ...
              {matrixHref && (
                <button onClick={() => router.push(matrixHref)} className="flex items-center gap-2 pb-2 text-[#6B7280] hover:text-[#111827]">
                  <ShieldCheck className="h-4 w-4" /> Permissions Matrix
                </button>
              )}
              {auditHref && (
                <button onClick={() => router.push(auditHref)} className="flex items-center gap-2 pb-2 text-[#6B7280] hover:text-[#111827]">
                          ...
                          <div className="absolute right-4 top-10 w-[160px] rounded-[10px] border border-[#E5E7EB] bg-white p-1 shadow-lg z-10">
                            {(() => {
                              const dynamicItems: any[] = [
                                ...
                                ...(canChangeBranch ? [{ label: "Change Branch", icon: GitBranch }] : []),
                              ]
              ...
              <div className="mt-2 rounded-[12px] border border-[#EEF1F6] bg-white p-4">
                ...
                <div className={`${smallText} mt-4 grid grid-cols-5 gap-4`}>
                  ...
                  {["Director", "Pastor", "Accountant", "Admin"].map((role) => (
                    <div key={`approve-budget-${role}`} className="flex items-center gap-2">
                      <div className={`h-3.5 w-3.5 rounded border border-[#D1D5DB] flex items-center justify-center ${granted(role, ["budget", "finance"], ["approve"]) ? "bg-[#3B5BDB]" : "bg-white"}`}>
                        {granted(role, ["budget", "finance"], ["approve"]) && <span className="h-2 w-2 rounded-sm bg-white" />}
                      </div>
                      <span className={granted(role, ["budget", "finance"], ["approve"]) ? "text-[#6B7280]" : "text-[#9CA3AF]"}>Approve Budget</span>
                    </div>
                  ))}
                </div>
                ...
                <div className={`${smallText} mt-4 grid grid-cols-5 gap-4`}>
                  ...
                  {["Director", "Pastor", "Accountant", "Admin"].map((role) => (
                    <div key={`edit-coa-${role}`} className="flex items-center gap-2">
                      <div className={`h-3.5 w-3.5 rounded border border-[#D1D5DB] flex items-center justify-center ${granted(role, ["chart", "coa", "account", "finance"], ["update", "edit", "configure"]) ? "bg-[#3B5BDB]" : "bg-white"}`}>
                        {granted(role, ["chart", "coa", "account", "finance"], ["update", "edit", "configure"]) && <span className="h-2 w-2 rounded-sm bg-white" />}
                      </div>
                      <span className={granted(role, ["chart", "coa", "account", "finance"], ["update", "edit", "configure"]) ? "text-[#6B7280]" : "text-[#9CA3AF]"}>Edit COA</span>
                    </div>
                  ))}
                </div>
                ...
                <div className={`${smallText} mt-4 grid grid-cols-5 gap-4`}>
                  <div />
                  {["Director", "Pastor", "Accountant", "Admin"].map((role) => (
                    <div key={`invite-users-${role}`} className="flex items-center gap-2">
                      <div className={`h-3.5 w-3.5 rounded border border-[#D1D5DB] flex items-center justify-center ${granted(role, ["user", "staff", "people"], ["create", "invite"]) ? "bg-[#3B5BDB]" : "bg-white"}`}>
                        {granted(role, ["user", "staff", "people"], ["create", "invite"]) && <span className="h-2 w-2 rounded-sm bg-white" />}
                      </div>
                      <span className={granted(role, ["user", "staff", "people"], ["create", "invite"]) ? "text-[#6B7280]" : "text-[#9CA3AF]"}>Invite Users</span>
                    </div>
                  ))}
                </div>

                <div className="mt-4 flex justify-end">
                  <button onClick={() => matrixHref && router.push(matrixHref)} disabled={!matrixHref} className={`flex items-center gap-1 ${smallText} text-[#3B5BDB] disabled:opacity-40`}>
