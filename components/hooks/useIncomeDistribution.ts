...
export function useIncomeDistribution(options: Options = {}) {
  const { user } = useAuth()
  const [items, setItems] = useState<IncomeDistributionItem[]>([])
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const url = query
          ? `/api/v1/core/dashboard/income-distribution?${query}`
          : "/api/v1/core/dashboard/income-distribution"
        const response = await fetch(url, { method: "GET", credentials: "include" })
        const payload = await response.json().catch(() => null)
        if (!response.ok) {
          throw new Error(payload?.message ?? "Unable to fetch income distribution.")
        }
        ...
        if (isMounted) {
          setError(err instanceof Error ? err.message : "Unable to fetch income distribution.")
          setItems([])
        }
      } finally {
